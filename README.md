...
Com a Fase 1 (fundação técnica) concluída e o catálogo de categorias estatutárias formalizado (P2-01), a próxima etapa é o cadastro de associados (P2-02), que inclui o vínculo entre associado e categoria estatutária. A P2-02 já foi **refinada** em [`docs/product/member-model-refinement.md`](docs/product/member-model-refinement.md) — modelo conceitual, matriz de campos e finalidades, pessoa física x jurídica, deduplicação e pacote de decisão — mas sua **implementação depende de decisões da ACASA** ainda pendentes (DP-008, DP-013, DP-014 e, para a fronteira com a P2-04, DP-005). Nenhuma migration de associado foi criada. Perfis/permissões administrativas (P2-05) e a inscrição pública (P2-08/P2-09) dependem de decisões ainda registradas como pendentes. Os ambientes Preview/Production ainda não existem e sua criação exige tarefa específica e decisão de custo. Entidades e campos de negócio só devem ser adicionados quando suas regras estiverem suficientemente definidas.

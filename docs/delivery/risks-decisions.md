...
| DP-015 | **PENDENTE — parcela DP-015B:** quem pode receber a capacidade de manter o cadastro e quem na ACASA autoriza concessões/revogações? | DT-015A resolve a parcela técnica: concessão específica por UUID, consultada no servidor e protegida por RLS; não define destinatários nem implementa acesso | **entrega utilizável da P2-02**: CRUD, telas e Server Actions. Fundação local isolada pode ser implementada em tarefa própria, com contas fictícias e tabelas de negócio fechadas. Fonte: [refinamento, Issue #22](../security/admin-authorization-refinement.md) |
...
**DT-015A — DECISÃO TÉCNICA:** mecanismo mínimo especificado no [refinamento de autorização](../security/admin-authorization-refinement.md), §§2–7. **DP-015B — DECISÃO DE PRODUTO PENDENTE:** pacote de decisão na §9. O fechamento da Issue #22 significa conclusão do refinamento, não resolução integral de DP-015. Nenhum órgão estatutário foi convertido automaticamente em papel de sistema.
